...
t_char *fillLimitKey(t_char *trueKey)
{
...
t_char encodeLimit(t_char val, t_char *limitKey)
{
    int index = val.get_value();
    index = index % 65;
    t_char returning = limitKey[index];
    return returning;
}
...
t_char decodeLimit(t_char val, t_char *limitKey)
{
    int i;
    bool found = false;
    for (i = 0; i < 26; i++)
    {
        if (val.get_value() == limitKey[i].get_value())
        {
            found = true;
            break;
        }
...
int main()
{
    ...
    t_char *limitKeys;
    limitKeys = fillLimitKey(limitKeys);
    ...
    for(int i = 0; i < 5; i++)
    {
        ...
        else if(plainText[i].get_Degree() == L)
        {
            cipherText[i] = encodeLimit(plainText[i] , limitKeys);
        }
        else
        {
    ...
    for(int i = 0; i< 5; i++)
    {
        ...
        else if(cipherText[i].get_Degree() == L)
        {
            decodedText[i] = decodeLimit(cipherText[i] , limitKeys);
        }
        else
        {
