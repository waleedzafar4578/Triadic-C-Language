#include "ttreenode_lint.h"
ttreenode_lint *bstree_insert(ttreenode_lint *root, ttreenode_lint &obj, tp_lint &val);
ttreenode_lint *bstree_delete(ttreenode_lint *root, long int key);
ttreenode_lint *bstree_insert(ttreenode_lint *root, Degree pointer_degree, long int value, Degree value_degree, tp_lint &val);
void bstree_print(ttreenode_lint *root);
class tbstree_lint
{
    ttreenode_lint *root = NULL;
...
public:
    ...
    //     dlist->right = nullptr;
    // }
    void insert_node(ttreenode_lint &obj)
    {
...
void t_scan(tbstree_lint &obj)
{
    obj.l_s();
}
void t_print(tbstree_lint &obj)
{
    obj.l_p();
}
ttreenode_lint *bstree_insert(ttreenode_lint *root, ttreenode_lint &obj, tp_lint &val)
...
ttreenode_lint *bstree_insert(ttreenode_lint *root, Degree pointer_degree, long int value, Degree value_degree, tp_lint &val)
{
    ...
    if (root == NULL)
    {
        return new ttreenode_lint(pointer_degree, value, value_degree);
    }
...
ttreenode_lint* minimum(ttreenode_lint* root)
{
    ttreenode_lint* current = root;
...
ttreenode_lint *bstree_delete(ttreenode_lint *root, long int key)
{
    ...
    else
    {
        if(root->left == NULL)
        {
            ttreenode_lint *temp = root->right;
            free(root);
            return temp;
        }
        else if(root->right == NULL)
        {
            ttreenode_lint* temp = root->left;
            free(root);
            return temp;
        }
        ttreenode_lint* temp = minimum(root->right);
        root->set_node(*temp);
