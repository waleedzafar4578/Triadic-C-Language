#include "tavlnode_int.h"
tavlnode_int *avltree_insert(tavlnode_int *root, tavlnode_int &obj, tp_int &val);
tavlnode_int *avltree_delete(tavlnode_int *root, int key);
tavlnode_int *avltree_insert(tavlnode_int *root, Degree pointer_degree, int value, Degree value_degree, tp_int &val);
void avltree_print(tavlnode_int *root);
class tavltree_int
{
    tavlnode_int *root = NULL;

public:
    int index = 0;
    tp_int d_base[10];
    void insert_node(tavlnode_int &obj)
    {
...
void t_scan(tavltree_int &obj)
{
    obj.l_s();
}
void t_print(tavltree_int &obj)
{
    obj.l_p();
}
tavlnode_int *avltree_insert(tavlnode_int *root, tavlnode_int &obj, tp_int &val)
...
tavlnode_int *avltree_insert(tavlnode_int *root, Degree pointer_degree, int value, Degree value_degree, tp_int &val)
{
    if (root == NULL)
    {
        return new tavlnode_int(pointer_degree, value, value_degree);
    }
...
tavlnode_int *minimum(tavlnode_int *root)
{
    tavlnode_int *current = root;
...
tavlnode_int *avltree_delete(tavlnode_int *root, int key)
{
    ...
    else
    {
        ...
        {
            tavlnode_int *temp = root->left ? root->left : root->right;
            if (temp == NULL)
            {
                temp = root;
                root = NULL;
        ...
        {
            tavlnode_int *temp = minimum(root->right);
            root->set_node(*temp);
