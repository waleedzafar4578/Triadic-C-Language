#include "tavlnode_float.h"
tavlnode_float *avltree_insert(tavlnode_float *root, tavlnode_float &obj, tp_float &val);
tavlnode_float *avltree_delete(tavlnode_float *root, float key);
tavlnode_float *avltree_insert(tavlnode_float *root, Degree pointer_degree, float value, Degree value_degree, tp_float &val);
void avltree_print(tavlnode_float *root);
class tavltree_float
{
    tavlnode_float *root = NULL;

public:
    int index = 0;
    tp_float d_base[10];
    void insert_node(tavlnode_float &obj)
    {
...
void t_scan(tavltree_float &obj)
{
    obj.l_s();
}
void t_print(tavltree_float &obj)
{
    obj.l_p();
}
tavlnode_float *avltree_insert(tavlnode_float *root, tavlnode_float &obj, tp_float &val)
...
tavlnode_float *avltree_insert(tavlnode_float *root, Degree pointer_degree, float value, Degree value_degree, tp_float &val)
{
    if (root == NULL)
    {
        return new tavlnode_float(pointer_degree, value, value_degree);
    }
...
tavlnode_float *minimum(tavlnode_float *root)
{
    tavlnode_float *current = root;
...
tavlnode_float *avltree_delete(tavlnode_float *root, float key)
{
    ...
    else
    {
        ...
        {
            tavlnode_float *temp = root->left ? root->left : root->right;
            if (temp == NULL)
            {
                temp = root;
                root = NULL;
        ...
        {
            tavlnode_float *temp = minimum(root->right);
            root->set_node(*temp);
