#include "ttreenode_float.h"
ttreenode_float *bstree_insert(ttreenode_float *root, ttreenode_float &obj, tp_float &val);
ttreenode_float *bstree_delete(ttreenode_float *root, float key);
ttreenode_float *bstree_insert(ttreenode_float *root, Degree pointer_degree, float value, Degree value_degree, tp_float &val);
void bstree_print(ttreenode_float *root);
class tbstree_float
{
    ttreenode_float *root = NULL;
...
public:
    ...
    //     dlist->right = nullptr;
    // }
    void insert_node(ttreenode_float &obj)
    {
    ...
    void insert_node(Degree pointer_degree, float value, Degree value_degree)
    {
...
void t_scan(tbstree_float &obj)
{
    obj.l_s();
}
void t_print(tbstree_float &obj)
{
    obj.l_p();
}
ttreenode_float *bstree_insert(ttreenode_float *root, ttreenode_float &obj, tp_float &val)
{
    ...
    if (root == NULL)
    {
        return new ttreenode_float(obj);
    }
...
ttreenode_float *bstree_insert(ttreenode_float *root, Degree pointer_degree, float value, Degree value_degree, tp_float &val)
{
    ...
    if (root == NULL)
    {
        return new ttreenode_float(pointer_degree, value, value_degree);
    }
    else if (root != NULL && pointer_degree == T)
    {
        t_float t2_1(value, value_degree);
        t_float t1(root->get_node().get_pointer().get_value(), root->get_node().get_pointer().get_Degree());
        t_float ans;
        ans = tlt(t1, t2_1);
        if (ans.get_Degree() == T && ans.get_value() == t1.get_value())
...
ttreenode_float* minimum(ttreenode_float* root)
{
    ttreenode_float* current = root;
...
ttreenode_float *bstree_delete(ttreenode_float *root, float key)
{
    ...
    else
    {
        if(root->left == NULL)
        {
            ttreenode_float *temp = root->right;
            free(root);
            return temp;
        }
        else if(root->right == NULL)
        {
            ttreenode_float* temp = root->left;
            free(root);
            return temp;
        }
        ttreenode_float* temp = minimum(root->right);
        root->set_node(*temp);
