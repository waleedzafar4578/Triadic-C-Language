...
void QuickSort(tarray_int &obj, int start, int end)
{
    if (start >= end)
        return;
    int p = partition(obj, start, end);
    QuickSort(obj, start, p - 1);
    QuickSort(obj, p + 1, end);
}
void t_quicksort_int(tarray_int &obj)
{
    int filter1_size = 0;
    ...
    QuickSort(filter2f , 0 , filter2f_size - 1);
    QuickSort(filter2l , 0 , filter2l_size - 1);
    QuickSort(filter2t , 0 , filter2t_size - 1);
