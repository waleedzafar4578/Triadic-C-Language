...
public:
    tqueue_double(int _size, Degree D)
    {
    ...
    void enqueue(double value, Degree d)
    {
        if (isFull())
        {
            cout << "Queue Overflow" << endl;
    ...
    double get_array_front_value()
    {
        return array.get_pointer_value(front);
    }
    //
    double get_array_rear_value()
    {
        return array.get_pointer_value(rear);
    }
    tarray_double get_array()
    {
        return array;
    }
    int get_size()
...
void scanQueue(tqueue_double &obj, int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        t_double val(output, d);
        obj.enqueue(val);
    }
}
void scanQueue(tqueue_double &obj)
{
    int _size;
...
void printQueue(tqueue_double &obj)
{
