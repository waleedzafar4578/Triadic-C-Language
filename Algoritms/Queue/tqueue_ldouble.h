#include "../Array/tarray_ldouble.h"
class tqueue_ldouble
{
private:
    tarray_ldouble array;
    int front;
    int rear;
    t_int size;

public:
    tqueue_ldouble(int _size, Degree D)
    {
    ...
    void enqueue(long double value, Degree d)
    {
        if (isFull())
        {
            cout << "Queue Overflow" << endl;
    ...
    long double get_array_front_value()
    {
        return array.get_pointer_value(front);
    }
    //
    long double get_array_rear_value()
    {
        return array.get_pointer_value(rear);
    }
    tarray_ldouble get_array()
    {
        return array;
    }
    int get_size()
...
void scanQueue(tqueue_ldouble &obj, int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        t_ldouble val(output, d);
        obj.enqueue(val);
    }
}
void scanQueue(tqueue_ldouble &obj)
{
    int _size;
...
void printQueue(tqueue_ldouble &obj)
{
